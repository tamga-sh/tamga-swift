...
let package = Package(
    ...
    targets: [
        ...
        .testTarget(
            ...
            // Real `.machine` files produced by the tamga-api server's own
            // `encode_machine_file`, plus the `manifest.json` describing them.
            // Bundled as a resource so CI is self-contained and needs no
            // network or server checkout.
            //
            // These are deliberately NOT generated by this repo. Every SDK in
            // this fleet previously tested its machine-file verifier against a
            // file it had encoded itself, which reproduced its own misreading
